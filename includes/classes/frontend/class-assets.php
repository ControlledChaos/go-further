...
class Assets {
	...
	public function frontend_styles() {
		 ...
		 */
		wp_enqueue_style( 'go-further', get_theme_file_uri( '/assets/css/style' . $this->suffix() . '.css' ), [ 'go-style' ], GF_VERSION, 'all' );

		// Right-to-left languages.
		if ( is_rtl() ) {
			wp_enqueue_style( 'go-further-rtl', get_theme_file_uri( 'assets/css/style-rtl' . $this->suffix() . '.css' ), [ 'go-further' ], GF_VERSION, 'all' );
		}

		// Block styles.
		if ( function_exists( 'has_blocks' ) ) {
			wp_enqueue_style( 'go-further-blocks', get_theme_file_uri( '/assets/css/blocks' . $this->suffix() . '.css' ), [ 'wp-block-library', 'go-further' ], GF_VERSION, 'all' );

			if ( is_rtl() ) {
				wp_enqueue_style( 'go-further-blocks-rtl', get_theme_file_uri( '/assets/css/blocks-rtl' . $this->suffix() . '.css' ), [ 'go-further-blocks' ], GF_VERSION, 'all' );
			}
		}

		// Print styles.
		wp_enqueue_style( 'go-further-print', get_theme_file_uri( '/assets/css/print' . $this->suffix() . '.css' ), [], GF_VERSION, 'print' );
	}
